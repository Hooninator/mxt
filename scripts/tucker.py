...
def read_tensor(args):
    tensor = torch.zeros(args.shape, dtype=torch.float64)
    with open(args.tensorpath, 'r') as file:
        for entry in file:
            stuff = entry.split(" ")[:-1]
...
def gpu_randn(d_X, r):
    return make_gaussian(d_X.shape[0], r, 'cuda', config.ttmc_u)
...
def init_factors_randn(X, ranks):
    order = X.ndim
    assert order == len(ranks)
    factors = []
    for k in range(order):
        X_k = unfold(X, k)
        U_k = gpu_randn(X_k, ranks[k])
        factors.append(U_k)
    return factors
...
def hooi_fast(X, ranks, maxiters):
    ...
    d_U_list = init_factors_randn(d_X, ranks)
...
def write_core(args, d_G):
    print(torch.nonzero(d_G).shape)
    inds = np.array(torch.nonzero(d_G.cpu()))
    print(inds)
    filename = f"core_{args.tensorpath.split('/')[-1]}"
    with open(filename, 'w') as file:
        for i in range(inds.shape[0]):
            indices = inds[i, :] + 1
            line = ' '.join(str(idx) for idx in indices)
            file.write(f"{line} {d_G[tuple(inds[i, :])]:.18f}\n")
...
if __name__ == "__main__":
    ...
    d_G = hooi_fast(X, ranks, args.maxiters)
